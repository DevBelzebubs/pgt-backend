...
public class PdfGeneratorService {
        ...
        try (PDDocument document = new PDDocument();
            ...
            try (PDPageContentStream contentStream = new PDPageContentStream(document, page)) {
                ...
                for (Product p : products) {
                    if (yPosition < MARGIN + 50) {
                        ...
                        try (PDPageContentStream newCs = new PDPageContentStream(document, newPage)) {
                            ...
                            yPosition = drawTableRow(newCs, p, tableX, yPosition, columnWidths, fontRegular, fontBold, products.indexOf(p) % 2 == 1);
                        }
                        yPosition -= 2;
                    } else {
                        yPosition = drawTableRow(contentStream, p, tableX, yPosition, columnWidths, fontRegular, fontBold, products.indexOf(p) % 2 == 1);
                        yPosition -= 2;
                    }
                }
                ...
                if (document.getPages().getCount() == 1) {
                    ...
                    drawFooter(contentStream, effectivePageWidth - MARGIN - 120, yPosition, fontItalic, products.size());
                }
            }
    ...
    private float drawTableHeader(PDPageContentStream cs, float x, float y, 
        ...
        String[] headers = {"SKU", "Cód. Anexo", "Producto", "Precio Compra", "Precio Venta", "Estado", "Fecha Creación"};
        float rowHeight = 25f;
